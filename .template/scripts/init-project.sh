...
if [ "$MODE" = "update" ]; then
    ...
    if [ "$OLD_VERSION" = "$TEMPLATE_VERSION" ]; then
        ...
        case $choice in
            1)
                ./.template/scripts/consolidate-agent-configs.sh
                ;;
            2)
...
if [ -f "README.md" ]; then
    mv README.md .template/docs/TEMPLATE_README.md.backup
    echo -e "${GREEN}✓ 已備份 → .template/docs/TEMPLATE_README.md.backup${NC}"
fi
echo ""
...
echo "   - .template/docs/AGENTS_MD_GUIDE.md - Agent skills 說明"
echo "   - .template/docs/DOCUMENTATION_GUIDELINES.md - 文件組織規範"
echo "   - .template/docs/TEMPLATE_SYNC.md - 模板同步指南"
echo ""
echo -e "${GREEN}✨ 完成！${NC}"
