...
# 解決多專案共用資料庫導致的衝突問題
#
# Usage: ./.template/scripts/init-opencode.sh
#
# 功能：
...
# 檢查是否有模板
if [ -f ".template/vscode/settings.json.template" ]; then
    cp .template/vscode/settings.json.template .vscode/settings.json
...
echo "  ${GREEN}./.template/scripts/init-opencode.sh${NC}"
echo ""
echo "  或手動複製："
echo "  ${GREEN}cp .template/vscode/settings.json.template <other-project>/.vscode/settings.json${NC}"
echo ""

# 8. 詳細文件連結
echo -e "${BLUE}📖 詳細文件：${NC}"
echo "  - ADR 0005: .template/docs/adr/0005-single-instance-opencode-workflow.md"
echo "  - 設定指南: .template/docs/OPENCODE_SETUP_GUIDE.md"
echo ""
