...
# Compares .template-version with .template/VERSION and syncs changes
...
Sync template updates from .template/ to project root.
...
NOTES:
    - Compares .template-version (current) with .template/VERSION (latest)
...
if [[ ! -d "$TEMPLATE_DIR" ]]; then
    log_error ".template/ directory not found. Are you in the project root?"
    exit 1
fi

# Check if VERSION file exists
if [[ ! -f "$VERSION_FILE" ]]; then
    log_error ".template/VERSION file not found."
    exit 1
fi
