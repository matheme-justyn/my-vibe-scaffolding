...
REQUIRED_FILES=(
    ...
    ".template/scripts/bump-version.sh"
...
SCRIPTS=(
    ...
    ".template/scripts/sync-template.sh"
    ".template/scripts/install-hooks.sh"
    ".template/scripts/health-check.sh"
    ".template/scripts/smart-cleanup.sh"
)
...
SERVICE_DETECTION_FILES=(
    ...
    ".template/docs/service-detection-protocol.md"
)
...
# Generate READMEs
if "$PROJECT_ROOT/.template/scripts/generate-readme.sh" > /dev/null 2>&1; then
    ...
    if [[ -f "$PROJECT_ROOT/README.md" ]] && [[ -f "$PROJECT_ROOT/README.zh-TW.md" ]]; then
        ...
        if grep -q "Why \`.template/\`" "$PROJECT_ROOT/README.md"; then
            log_success "  → .template/ rationale section present"
        else
            log_error "  → .template/ rationale section missing"
        fi
...
if rsync -a --exclude=".git" --exclude=".opencode-data" --exclude="node_modules" \
    "$PROJECT_ROOT/.template/" "$TEST_DIR/" > /dev/null 2>&1; then
