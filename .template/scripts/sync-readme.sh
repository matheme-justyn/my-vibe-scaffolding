#!/bin/bash
# sync-readme.sh - Sync all README.{locale}.md files from root to .template/ (scaffolding mode only)
#
# 用途：在鷹架模式下，自動同步根目錄所有 README*.md 到 .template/
# Purpose: Sync all README*.md files from root to .template/ in scaffolding mode
#
# 新增功能 (2026-03-10):
...
echo -e "${YELLOW}正在同步 README 檔案到 .template/...${NC}"
echo ""
...
echo "  git commit -m \"chore: sync README files to .template/\""
