...
cp .template/vscode/settings.json.template /path/to/other-project/.vscode/settings.json

# 或使用 init 腳本
cp .template/scripts/init-opencode.sh /path/to/other-project/
...
# 如果太多，使用智慧清理
./.template/scripts/fix-opencode-stability.sh
```
...
### 腳本
- `.template/scripts/init-opencode.sh` - 自動化初始化
- `.template/scripts/fix-opencode-stability.sh` - 穩定性修復
- `.template/vscode/settings.json.template` - 配置模板
